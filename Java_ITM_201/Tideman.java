...
public class Tideman {
    ...
    // Check if there's a path from orignal winner back to themselves
    public static boolean check_cycle(int current_loser, int original_winner) {
        if (current_loser == original_winner) {
            return true;
        }
        else {
            for (int j = 0; j < candidate_count; j++) {
                // Check if the current loser is locked over all the other candidates
                if (locked[current_loser][j]) {
                    // Check if said candidate (new loser) is locked with the original winner
                    if (check_cycle(j, original_winner)) {
                        return true;
                    }
                }
            }
    ...
    public static void print_winner() {
        // Assume that candidate i is the winner
        for (int i = 0; i < candidate_count; i++) {
            boolean is_source = true;
            // Source candidate must not be locked as loser in any pairs
            for (int j = 0; j < candidate_count; j++) {
                if (locked[j][i]) {
                    is_source = false;
                    break;
